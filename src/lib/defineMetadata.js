...
export default function defineMetadata({ data }) {
  ...
  const siteFaviconUrl = siteFavicon ? imageBuilder.image(siteFavicon).width(256).height(256).url() : '/favicon.ico'
  ...
  const shareGraphic = page?.sharing?.shareGraphic?.asset || site?.sharing?.shareGraphic?.asset || ''
  const shareGraphicUrl = shareGraphic ? imageBuilder.image(shareGraphic).url() : false
