...
import { getBlogDetailPage, getPagesPaths } from '@/sanity/lib/fetch';
...
export async function generateMetadata({ params }) {
	...
	const data = await getBlogDetailPage({ queryParams: params, isPreviewMode });
	return defineMetadata({ data });
}

export default async function Page({ params }) {
	const isPreviewMode = draftMode().isEnabled;
	const pageData = await getBlogDetailPage({
		queryParams: params,
		isPreviewMode,
	});
	const { page } = pageData || {};
