...
import { apiVersion, dataset, previewSecretId, projectId } from './src/sanity/env'
...
const allowDuplicateDocumentTypes = ['pGeneral', 'pBlog']
...
const devPlugins = [...commonPlugins, visionTool({ defaultApiVersion: apiVersion })]

export default defineConfig({
  basePath: '/sanity',
  title: 'Michelle Minsuh Kwon Portfolio',
  projectId,
  dataset,
  ...
  document: {
    ...
    actions: (prev, context) => {
      ...
      return prev.map((originalAction) => {
        if (originalAction.action === 'duplicate' && !allowDuplicateDocumentTypes.includes(schemaType)) {
          return () => null
        }
